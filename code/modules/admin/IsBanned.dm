...
/world/IsBanned(key,address,computer_id)
	...
	if(config.ban_legacy_system)
		...
		var/DBQuery/query = dbcon.NewQuery("SELECT ckey, ip, computerid, a_ckey, reason, expiration_time, duration, bantime, bantype FROM [format_table_name("ban")] WHERE (ckey = '[ckeytext]' [ipquery] [cidquery]) AND (bantype = 'SOFT_PERMABAN' OR bantype = 'PERMABAN' OR bantype = 'ADMIN_PERMABAN' OR ((bantype = 'SOFT_TEMPBAN' OR bantype = 'TEMPBAN' OR bantype = 'ADMIN_TEMPBAN') AND expiration_time > Now())) AND isnull(unbanned)")
